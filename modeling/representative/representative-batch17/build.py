...
for S in DATA:
 ...
 for ti,T in enumerate(S['towers']):
  ...
  for ei,(a,b) in enumerate(edges):
   ...
   if endwall:
    fac.edge(lerp(a,b,.08),lerp(a,b,.92),zbase,h-1,2 if code=='A13379001' else 0,.08)
    if code=='A13610202':fac.edge(a,b,zbase,h*.30,2,.08,.12)
    ...
    if False:fac.edge(a,b,zbase,h-.2,2,.10)
    ...
    for k in range(bays):
     ...
     for f in range(floors):
      ...
      fac.edge(lerp(a,b,(u+v)/2-.0025),lerp(a,b,(u+v)/2+.0025),z+fh*.24,min(h-.15,z+fh*.86),1,.04,.36)
   ...
   if r['crown']=='frame' and L>=longest*.8:
    roof.edge(a,b,h+2.5,h+3.15,5,.75)
    for t in [.04,.96]:roof.edge(lerp(a,b,max(0,t-.025)),lerp(a,b,min(1,t+.025)),h,h+3.0,5,.45)
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'representative-batch17.blend'))
