 ...
 for ti,T in enumerate(S['towers']):
  ...
  for ei,(a,b) in enumerate(edges):
   ...
   if endwall:
    ...
    if r.get('green_band'):
     fac.edge(a,b,h*.55,h*.62,2,.08,.15)
    ...
    for f in range(floors) if ei%3==0 and not r.get('no_end_windows') else []:
     z=zbase+f*fh;fac.edge(lerp(a,b,.44),lerp(a,b,.57),z+fh*.3,z+fh*.82,3,.04,.3)
   else:
    if ei%3==1 and not r.get('red_endframe') and not r.get('green_band'):fac.edge(a,b,zbase,h-.2,2,.10)
    if r['style'] in ['corridor','mido'] and (ei%2==0 or r.get('continuous_bands')):
     for f in range(floors):
      z=zbase+f*fh;fac.edge(a,b,z+fh*.43,z+fh*.88,3,.04,.14);fac.edge(a,b,z+fh*.08,z+fh*.43,1,.03,.21)
     for k in range(1,max(2,round(L/2.3))):
      t=k/max(2,round(L/2.3));fac.edge(lerp(a,b,t-.06/L),lerp(a,b,t+.06/L),zbase,h,2 if code=='A15884703' else 0,.05,.22)
     bays=0
    for k in range(bays):
   ...
   for t in []: # No unsupported ground-floor pilotis
   ...
   if r['crown']=='frame' and L>=longest*.95 and L>7:
    roof.edge(a,b,h+2.5,h+3.15,5 if code=='A13410004' else 1,.48)
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'representative-batch11.blend'))
