   ...
   if endwall:
    ...
    for k in range(bays):
     ...
     for f in range(floors):
      ...
      fac.edge(lerp(a,b,(u+v)/2-.025/bays),lerp(a,b,(u+v)/2+.025/bays),z+fh*.24,min(h-.15,z+fh*.86),1,.04,.36)
...
def aim(cx,cy,z,span):
 co.location=(cx+span*.9,cy-span*1.1,z+span*.85);co.rotation_euler=(Vector((cx,cy,z))-co.location).to_track_quat('-Z','Y').to_euler();cam.ortho_scale=span*1.75;cam.clip_end=20000
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'representative-batch06.blend'))
