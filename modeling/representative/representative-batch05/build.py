...
for S in DATA:
 ...
 for ti,T in enumerate(S['towers']):
  ...
  for ei,(a,b) in enumerate(edges):
   ...
   if endwall:
    fac.edge(lerp(a,b,.08),lerp(a,b,.92),zbase,h-1,2 if r.get('end_accent') else 0,.08)
    ...
    for f in range(floors) if ei%3==0 and r.get('blank_windows',True) else []:
     z=zbase+f*fh;fac.edge(lerp(a,b,.44),lerp(a,b,.57),z+fh*.3,z+fh*.82,3,.04,.3)
   else:
    if r.get('full_face_accent',False) and ei%3==1:fac.edge(a,b,zbase,h-.2,2,.10)
    ...
    for k in range(bays):
     ...
     for f in range(floors):
      ...
      fac.edge(lerp(a,b,(u+v)/2-.012),lerp(a,b,(u+v)/2+.012),z+fh*r.get('window_bottom',.24),min(h-.15,z+fh*r.get('window_top',.86)),1,.04,.36)
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'representative-batch05.blend'))
