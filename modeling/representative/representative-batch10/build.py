...
for S in DATA:
 ...
 for ti,T in enumerate(S['towers']):
  ...
  for ei,(a,b) in enumerate(edges):
   ...
   if endwall:
    fac.edge(lerp(a,b,.08),lerp(a,b,.92),zbase,h-1,2 if r.get('dark_endwalls') else 0,.08)
    ...
    for f in range(floors) if ei%3==0 and not r.get('solid_endwalls') else []:
  ...
  if r.get('green_roofs'):roof.prism(shrink(pts,.98),h+.05,h+.18,5)
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'representative-batch10.blend'))
