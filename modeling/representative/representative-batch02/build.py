...
for S in DATA:
 ...
 for ti,T in enumerate(S['towers']):
  ...
  for ei,(a,b) in enumerate(edges):
   ...
   if endwall:
    fac.edge(lerp(a,b,.08),lerp(a,b,.92),zbase,h-1,2 if r.get('red_endframe') and code=='A10024828' else 0,.08)
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'representative-batch02.blend'))
