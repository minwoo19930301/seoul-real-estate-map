...
DATA=json.loads((P/'site-footprints.json').read_text())['sites']
...
for S in DATA:
 ...
 mats.append(material(code+' roof planting',(.24,.38,.09)))
 for ti,T in enumerate(S['towers']):
  ...
  body.prism(pts,zbase,h,0);body.prism(shrink(pts,.91),.2,zbase,4)
  edges=list(zip(pts,pts[1:]+pts[:1]));longest=max(math.dist(a,b) for a,b in edges)
  for ei,(a,b) in enumerate(edges):
   L=math.dist(a,b)
   if L<2:continue
   bays=max(1,round(L/r.get('bay',3.1)));endwall=L<longest*r.get('blank_ratio',.55)
   if endwall:
    fac.edge(lerp(a,b,.08),lerp(a,b,.92),zbase,h-1,0,.08)
    for f in range(floors) if ei%3==0 else []:
     z=zbase+f*fh;fac.edge(lerp(a,b,.44),lerp(a,b,.57),z+fh*.3,z+fh*.82,3,.04,.3)
   else:
    if ei%3==1:fac.edge(a,b,zbase,h-.2,2,.10)
    for k in range(bays):
     ...
     for f in range(floors):
      ...
      fac.edge(lerp(a,b,(u+v)/2-.012),lerp(a,b,(u+v)/2+.012),z+fh*.24,min(h-.15,z+fh*.86),1,.04,.36)
      if r.get('balconies') and k%3==1 and ei%2==0:
       fac.edge(lerp(a,b,u-.06/bays),lerp(a,b,v+.06/bays),z+.12,z+.32,1,.95)
       fac.edge(lerp(a,b,u-.06/bays),lerp(a,b,v+.06/bays),z+.32,z+1.15,3,.04,.9)
      if r['style']=='bands':fac.edge(lerp(a,b,k/bays),lerp(a,b,(k+1)/bays),z,z+.24,1,.28)
      if r['style']=='frames' and f%3==0:fac.edge(lerp(a,b,k/bays+.025),lerp(a,b,(k+1)/bays-.025),z,z+.35,2,.23)
     if r['style'] in ['frames','piers'] and k%2==0:fac.edge(lerp(a,b,k/bays),lerp(a,b,min(1,k/bays+.08)),zbase,h,2,.28)
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'representative-batch01.blend'))
