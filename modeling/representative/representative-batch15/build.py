...
for S in DATA:
 ...
 for ti,T in enumerate(S['towers']):
  ...
  for ei,(a,b) in enumerate(edges):
   L=math.dist(a,b)
   if L<2:continue
   bays=max(1,round(L/r.get('bay',3.1)));endwall=L<min(r.get('blank_max',16),longest*r.get('blank_ratio',.55))
   if endwall:
    ...
    for k in range(bays):
     ...
     for f in range(floors):
      ...
      fac.edge(lerp(a,b,(u+v)/2-.025/max(1,bays)),lerp(a,b,(u+v)/2+.025/max(1,bays)),z+fh*r.get('window_bottom',.24),min(h-.15,z+fh*r.get('window_top',.86)),1,.04,.36)
  ...
  if code=='A13509009':
   # Open crown frame, aligned to dominant source edge; no invented tower identity.
   def crownpoint(u,v):return (cx+ux*u-uy*v,cy+uy*u+ux*v)
   for v in [-4.5,4.5]:
    roof.edge(crownpoint(-9,v),crownpoint(9,v),h+5.8,h+6.7,5,.5)
    for u in [-8,-4,0,4,8]:
     roof.prism([crownpoint(u-.22,v-.22),crownpoint(u+.22,v-.22),crownpoint(u+.22,v+.22),crownpoint(u-.22,v+.22)],h,h+6.2,1)
   for u in [-9,9]:roof.edge(crownpoint(u,-4.5),crownpoint(u,4.5),h+5.8,h+6.7,5,.5)
...
def aim(cx,cy,z,span):
 co.location=(cx+span*.9,cy-span*1.1,z+span*.85);co.rotation_euler=(Vector((cx,cy,z))-co.location).to_track_quat('-Z','Y').to_euler();cam.ortho_scale=span*1.7;cam.clip_end=20000
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'representative-batch15.blend'))
