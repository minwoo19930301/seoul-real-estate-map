...
for S in DATA:
 ...
 for ti,T in enumerate(S['towers']):
  ...
  floors=floors or max(2,round(h/3));zbase=0.0;fh=(h-zbase)/floors
  ...
  body.prism(pts,0,h,0)
  edges=list(zip(pts,pts[1:]+pts[:1]));longest=max(math.dist(a,b) for a,b in edges)
  for ei,(a,b) in enumerate(edges):
   L=math.dist(a,b)
   if L<2:continue
   bays=max(1,round(L/r.get('bay',3.1)));da,db=max(edges,key=lambda e:math.dist(*e));alignment=abs(((b[0]-a[0])*(db[0]-da[0])+(b[1]-a[1])*(db[1]-da[1]))/(L*longest));endwall=L<longest*r.get('blank_ratio',.55) and alignment<.45
   if endwall:
    fac.edge(lerp(a,b,.08),lerp(a,b,.92),zbase,h-1,(2 if ti%2 else 5) if code=='A10024831' else 0,.08)
    if r.get('end_accent'):
     w=r.get('end_accent_width',.2);fac.edge(lerp(a,b,(1-w)/2),lerp(a,b,(1+w)/2),zbase,h-.4,2,.06,.18)
    if r.get('end_bands'):
     for fl in range(0,floors,3):fac.edge(a,b,fl*fh,fl*fh+.50,2,.025,.25)
    if r.get('top_accent'):fac.edge(lerp(a,b,.35),lerp(a,b,.65),h-6,h-.4,2,.03,.27)
    ...
    for f in range(floors) if ei%3==0 else []:
     z=zbase+f*fh;fac.edge(lerp(a,b,.44),lerp(a,b,.57),z+fh*.3,z+fh*.82,3,.04,.3)
   else:
    if r.get('full_face_accent') and ei%3==1:fac.edge(a,b,zbase,h-.2,2,.10)
    ...
    for k in range(bays):
     ...
     for f in range(floors):
      z=zbase+f*fh
      fac.edge(lerp(a,b,u),lerp(a,b,v),z+fh*r.get('window_bottom',.24),min(h-.15,z+fh*r.get('window_top',.86)),3,.10,.25)
      fac.edge(lerp(a,b,(u+v)/2-.055/L),lerp(a,b,(u+v)/2+.055/L),z+fh*.24,min(h-.15,z+fh*.86),1,.04,.36)
   ...
   roof.edge(a,b,h,h+.24,5 if r.get('cap_overhang') else 1,.65 if r.get('cap_overhang') else .20)
  ...
  roof.prism(rr,h,h+r.get('roof_height',2),5)
  if r['crown']=='pergola':
   for u in [-2.6,2.6]:
    q=(cx+ux*u,cy+uy*u);roof.prism([(q[0]-.25,q[1]-.25),(q[0]+.25,q[1]-.25),(q[0]+.25,q[1]+.25),(q[0]-.25,q[1]+.25)],h+1.8,h+3.4,1)
   roof.prism(rr,h+3.4,h+3.7,5)
  if r['crown']=='gabled_core':
   def gp(u,v,z):return (cx+ux*u-uy*v,cy+uy*u+ux*v,z)
   roof.face([gp(-3,-2.4,h+2),gp(3,-2.4,h+2),gp(3,0,h+3),gp(-3,0,h+3)],1)
   roof.face([gp(-3,0,h+3),gp(3,0,h+3),gp(3,2.4,h+2),gp(-3,2.4,h+2)],1)
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'representative-batch08.blend'))
