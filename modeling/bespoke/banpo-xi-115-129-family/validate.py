...
R=pathlib.Path.cwd();P=R/'data/model-source/bespoke/banpo-xi-115-129-family';sys.path.insert(0,str(R/'scripts/bespoke'));from validate_height_regions import read_triangles,above_ceiling
...
for t in map(str,range(115,130)):
 ...
 for region,part in zip(regions,parts):
  deck=region['body_height_m'];rt=tri[np.max(np.abs(tri[:,:,1]-deck),axis=1)<1e-4];cap=unary_union([Polygon(q[:,[0,2]]*[1,-1]) for q in rt]);inside=part.buffer(-.03);coverage=1-inside.difference(cap).area/inside.area;assert coverage>.999999,(t,region['id'],coverage);deck_checks.append({'region':region['id'],'deck_height_m':deck,'deck_coverage_ratio':coverage})
 result={'tower':t,'source_id':d['id'],'roof_deck_checks':deck_checks,'source_geometry_exact':True,'register_row_exact':True,'source_ring_local_area_m2':poly.area,'source_anchor_lonlat':d['anchor_lonlat'],'bounds_gltf_x_up_south':[xyz.min(axis=0).tolist(),xyz.max(axis=0).tolist()],'maximum_surface_vertex_projection_m':projection,'grade_missing_source_area_m2':missing,'grade_extra_decor_area_m2':sur.difference(poly).area,'regions_partition_gap_area_m2':unary_union(parts).symmetric_difference(poly).area,'regions_overlap_area_m2':parts[0].intersection(parts[1]).area,'lower_envelope_exceedance_area_m2':sum(bad),'height_m':d['register_height_m'],'floors_official':d['floors'],'lower_floors_inferred':d['lower_wing_floors_inferred'],'region_boundaries_inferred':True,'images':0,'scene_count':1,'triangle_count':len(tri),'tolerance_m':.35,'tolerance_basis':'Facade maximum intended offset .25 + half thickness .06 = .31m; .04 numerical/junction allowance, checked actual projection <=.35m'}
