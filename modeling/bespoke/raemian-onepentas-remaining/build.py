"""One Pentas101-104 inferred from approved105/106 family. Individually mapped actual rings and official unit-height arms.
...
def facade_106_east(asset,a,b,zlo,zhi,style,tag):
 ...
 bounds=sorted(set([zlo,zhi]+[4.05+k*D['towers'][n]['storeyPitchEstimateM'] for k in range(36)if zlo<4.05+k*D['towers'][n]['storeyPitchEstimateM']<zhi]));dark.face(a,b,zlo,zhi,.04,.04)
...
def facade_105_numbered_side(asset,a,b,zlo,zhi,tag):
 ...
 bounds=sorted(set([zlo,zhi]+[4.05+k*D['towers'][n]['storeyPitchEstimateM'] for k in range(36)if zlo<4.05+k*D['towers'][n]['storeyPitchEstimateM']<zhi]));dark.face(a,b,zlo,zhi,.04,.05)
...
def facade(n,asset,a,b,zlo,zhi,style,tag):
 ...
 bounds=sorted(set([zlo,zhi]+[4.05+k*D['towers'][n]['storeyPitchEstimateM'] for k in range(36)if zlo<4.05+k*D['towers'][n]['storeyPitchEstimateM']<zhi]))
 if style=='recess':
...
# Roof-only v4: reviewed 105/106 mechanical-head family; main high roofs only.
for n,rec in D['towers'].items():
 if 'serviceHead' not in rec:continue
 spec=rec['serviceHead'];asset='bespoke-raemian-onepentas-'+n;c='OnePentas_'+n+'_05_photo_serviceheads'
 x,y=spec['center'];w,d=spec['width'],spec['depth'];z=spec['base'];top=z+spec['height'];angle=spec['angle']
 u=Vector((math.cos(angle),math.sin(angle)));v=Vector((-u.y,u.x));ce=Vector((x,y));r=ccw([tuple(ce+u*xx+v*yy) for xx,yy in [(-w/2,-d/2),(w/2,-d/2),(w/2,d/2),(-w/2,d/2)]])
 P(n+'_roof_servicehead','dark_panel',c,asset).prism(r,z,top)
 for aa,bb in zip(r,r[1:]+r[:1]):
  aa,bb=Vector(aa),Vector(bb);L=(bb-aa).length;vv=(bb-aa).normalized()
  for j in range(max(3,round(L/.9))+1):
   q=aa+(bb-aa)*j/max(3,round(L/.9));P(n+'_roof_vertical_metal_fins','bronze',c,asset).face(q-vv*.05,q+vv*.05,z,top,.15,.09)
  P(n+'_thin_copper_top','bronze_light',c,asset).face(aa,bb,top-.12,top+.10,.35,.12)

asset='bespoke-raemian-onepentas-101';c='OnePentas_101_06_shared_skycommunity_owner101'
a=Vector((29,-57));b=Vector((22,-94));u=(b-a).normalized();v=Vector((-u.y,u.x));r=ccw([tuple(a+v*4.2),tuple(b+v*4.2),tuple(b-v*4.2),tuple(a-v*4.2)])
P('101_102_shared_bridge_floor','bronze',c,asset).prism(r,96.55,97.05)
P('101_102_shared_bridge_glass','glass',c,asset).prism(r,97.05,100.95)
P('101_102_shared_bridge_roof','bronze_light',c,asset).prism(r,100.95,101.30)
for aa,bb in zip(r,r[1:]+r[:1]):
 aa,bb=Vector(aa),Vector(bb);delta=bb-aa;count=max(2,round(delta.length/1.5))
 for j in range(count+1):
  q=aa+delta*j/count;P('101_102_shared_bridge_mullions','dark_panel',c,asset).beam((q.x,q.y,97.05),(q.x,q.y,100.95),.11)

# Photo00 completed bridge: bronze rails and diagonal glazed truss; landingheads reach both towers.
for endpoint,owner in [(a,'101'),(b,'102')]:
 # a/b vectors above remain original bridge endpoints (aa/bb are edge loops).
 aid='bespoke-raemian-onepentas-'+owner;cc='OnePentas_'+owner+'_07_bridge_landing'
 base=4.05+(29*D['towers'][owner]['storeyPitchEstimateM'])
 P(owner+'_bridge_landing_core','dark_panel',cc,aid).box(endpoint.x,endpoint.y,(base+103.0)/2,7.2,8.0,103.0-base,.33)
 for dx in [-3.1,-2.0,-.9,.2,1.3,2.4,3.1]:
  P(owner+'_landing_bronze_fins','bronze',cc,aid).box(endpoint.x+dx,endpoint.y-4.08,(base+103.0)/2,.09,.17,103.0-base)
 P(owner+'_landing_cap','bronze_light',cc,aid).box(endpoint.x,endpoint.y,103.05,7.55,8.35,.10,.33)
for sign in [-1,1]:
 for j in range(9):
  aa=a+(b-a)*j/9+v*4.24;bb=a+(b-a)*(j+1)/9+v*4.24
  if sign==-1:aa-=v*8.48;bb-=v*8.48
  P('101_bridge_glazed_diagonal_truss','bronze_light',c,asset).beam((aa.x,aa.y,97.2 if j%2==0 else 100.8),(bb.x,bb.y,100.8 if j%2==0 else 97.2),.18)
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'raemian-onepentas-remaining.blend'));print(json.dumps(validation))
