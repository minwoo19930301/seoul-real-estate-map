...
P=pathlib.Path(__file__).parent;D=json.loads((P/'source-data.json').read_text());regions=json.loads((P/'regions.json').read_text());old=bpy.context.scene;before={s.name:sorted(o.name for o in s.objects) for s in bpy.data.scenes};s=bpy.data.scenes.new('Godeok Geumho101 representative');bpy.context.window.scene=s
...
def mat(n,c,metal=0,rough=.65):
 m=bpy.data.materials.new('Geumho101 '+n);m.diffuse_color=(*c,1);m.use_nodes=True;q=m.node_tree.nodes.get('Principled BSDF');q.inputs['Base Color'].default_value=(*c,1);q.inputs['Metallic'].default_value=metal;q.inputs['Roughness'].default_value=rough;return m
...
# Geumho photo: ivory paired window banks, grey service piers, restrained terracotta ends.
charcoal=mat('grey service pier',(.37,.40,.39));warm=mat('muted terracotta accents',(.44,.25,.15));roles=[]
...
for ei,(a,b) in enumerate(zip(r,r[1:]+r[:1])):
 ...
 main=L>=10;small=L<3.5
 roles.append({'edge':ei,'length_m':L,'role':'paired ivory framed glazing banks with grey service center' if main else ('solid return' if small else 'grey service face with small punched windows'),'basis':'ownedge width classification, unmeasured complex inference'})
 if L<.25:continue
 face(.5,body/2,L,body,.012,white if main or small else charcoal,depth=.024)
 face(.5,pitch,L,2*pitch,.035,metal,depth=.07)
 if ei==D['terracotta_end_edge_inferred']:
  face(.5,body/2,L-.48,body,.085,warm,depth=.10)
  for t in [.018,.982]:face(t,body/2,.34,body,.155,white,depth=.12)
  face(.5,body-.30,L,.60,.16,white,depth=.12)
  roles[-1]['role']='full-height broad opaque terracotta wing-end with pale perimeter frame'
 elif main:
  face(.5,body/2,L*.24,body,.045,charcoal,depth=.045)
  for lo,hi in [(.035,.36),(.64,.965)]:
   mid=(lo+hi)/2;bw=(hi-lo)*L
   for f in range(FLOORS):
    z=(f+.56)*pitch;face(mid,z,bw-.25,.61*pitch,.09,glass,depth=.035)
    for t in [mid-.22*bw/L,mid+.22*bw/L]:face(t,z,.055,.61*pitch,.135,white,depth=.035)
    face(mid,f*pitch+.24,bw,.48,.14,white,depth=.10)
   for t in [lo,hi]:face(t,body/2,.25,body,.155,white,depth=.12)
  for f in range(FLOORS):face(.5,(f+.55)*pitch,min(.65,L*.05),.37*pitch,.08,glass,depth=.035)
 elif not small:
  for f in range(FLOORS):
   for t in [.32,.68]:face(t,(f+.55)*pitch,min(.7,L*.12),.4*pitch,.065,glass,depth=.035)
  if L>6:face(.93,body/2,.30,body,.065,warm,depth=.05)
 for f in range(1,FLOORS):face(.5,f*pitch,L-.08,.015,.026,metal,depth=.01)
 face(.5,body-.15,L,.30,.06,white,depth=.08)
...
box((x,y,H-.10),(w+.10,d+.10,.20),warm,theta)
objs=[]
for m,(v,f) in batches.items():
 me=bpy.data.meshes.new('Geumho101 editable '+m.name);me.from_pydata(v,[],f);me.update();ob=bpy.data.objects.new('Geumho101 editable '+m.name,me);s.collection.objects.link(ob);me.materials.append(m);objs.append(ob)
...
bpy.context.view_layer.objects.active=objs[0];bpy.ops.export_scene.gltf(filepath=str(P/'geumho-park-hills-101.glb'),use_selection=True,use_active_scene=True,export_format='GLB',export_yup=True)
s.render.engine='CYCLES';s.cycles.samples=20;s.render.resolution_x=850;s.render.resolution_y=1100;s.render.resolution_percentage=100;s.world=bpy.data.worlds.new('Geumho101 world');s.world.use_nodes=True;s.world.node_tree.nodes['Background'].inputs[0].default_value=(.72,.77,.82,1);s.world.node_tree.nodes['Background'].inputs[1].default_value=.75
ld=bpy.data.lights.new('Geumho101 sun','SUN');lo=bpy.data.objects.new('Geumho101 sun',ld);s.collection.objects.link(lo);lo.rotation_euler=(.45,-.4,-.6);ld.energy=2.3;s.view_settings.view_transform='AgX'
for name,(u,v),z in [('front',(75,-155),H),('opposite',(-75,155),H),('side',(170,50),H),('roof',(65,-85),H*2.55)]:
 ca=bpy.data.cameras.new('Geumho101 '+name);ob=bpy.data.objects.new('Geumho101 '+name,ca);s.collection.objects.link(ob);ob.location=(u*co-v*si,u*si+v*co,z);ob.rotation_euler=(Vector((0,0,H/2))-ob.location).to_track_quat('-Z','Y').to_euler();ca.type='ORTHO';ca.ortho_scale=H*1.23;inv=ob.rotation_euler.to_matrix().transposed();q=[inv@(o.matrix_world@vv.co-ob.location) for o in objs for vv in o.data.vertices];ca.ortho_scale=max(ca.ortho_scale,2*max(abs(v.y) for v in q)*1.12,2*max(abs(v.x) for v in q)/(850/1100)*1.12);s.camera=ob;s.render.filepath=str(P/(name+'.png'));bpy.ops.render.render(write_still=True)
vs=[o.matrix_world@v.co for o in objs for v in o.data.vertices];report={'source_id':D['id'],'floors':D['floors'],'register_height_m':D['register_height_m'],'osm_height_m':D['osm_height_m'],'height_status':D['height_status'],'uniform_height_assumption':True,'bounds_xyz':[[min(v[i] for v in vs) for i in range(3)],[max(v[i] for v in vs) for i in range(3)]],'mesh_count':len(objs),'image_nodes':0,'finite':all(math.isfinite(c) for v in vs for c in v)};assert abs(report['bounds_xyz'][0][2])<1e-5 and abs(report['bounds_xyz'][1][2]-H)<1e-4 and report['finite'];(P/'validation.json').write_text(json.dumps(report,indent=2));bpy.data.libraries.write(str(P/'geumho-park-hills-101-authored.blend'),{s},fake_user=True,compress=True);bpy.context.window.scene=old;bpy.data.batch_remove(ids=list(s.objects));bpy.data.scenes.remove(s);after={s.name:sorted(o.name for o in s.objects) for s in bpy.data.scenes};assert before==after;(P/'scene-preservation.json').write_text(json.dumps({'equal':before==after,'initial_scene':old.name,'before':before,'after':after},indent=2));print(json.dumps(report))
