...
assets=[];mcps=[]
for number in list(range(108,115))+[201,202]:
 ...
 source['facadeRelief']={'nominalProjectedCoreFaceM':1.325 if len(ring)==6 else 0,'outermostCladdingJointM':1.342 if len(ring)==6 else 0,'basis':'Representative inference; measured GLB bounds below include all modeled relief.'}
...
b={'siteId':'maple-108-202-family','sources':D['sources'],'assets':assets,'places':[],'recipeFiles':[p.name for p in O.iterdir()if p.suffix=='.py'and not p.name.startswith('v1-')]+[p.name for p in O.iterdir()if p.name.startswith(('input-','source-anchor-','geometry-validation-','glb-validation-','build-summary-'))],'mcpEvidence':mcps,'reviewStatus':'final-v2-candidate-awaiting-root-review'}
