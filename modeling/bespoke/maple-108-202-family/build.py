"""Maple 108–114, 201–202 representative-family inference on each retained numbered plan. No individual photo-face identity claimed."""
import bpy,bmesh,json,math
from pathlib import Path
from mathutils import Vector
from mathutils.geometry import tessellate_polygon
O=Path(__file__).parent;NUMBER=globals().get('NUMBER',108);D=json.loads((O/f'input-{NUMBER}.json').read_text());V=[Vector(p)for p in D['ringEN']];N=len(V)
...
for e in range(N):
 ...
 if role=='core':
  ...
  if NUMBER==113 and e==2:cols=[(.28,.34,False,True),(.56,.62,False,True),(.82,.88,False,False)]
 else:
