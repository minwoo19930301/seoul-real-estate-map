...
assert len(objs)>80 and all(x['volume']>0 and x['nonManifold']==0 and x['finite']and x['identityTransform']for x in checks)
...
for sample in B['windowSamples']:
 e=sample['e'];a,b=V[e],V[(e+1)%len(V)];n=Vector((-(b-a).y,(b-a).x)).normalized();p=a+(b-a)*sample['t'];start=Vector((p.x+n.x*4,p.y+n.y*4,sample['z']));hit=bpy.context.scene.ray_cast(bpy.context.evaluated_depsgraph_get(),start,Vector((-n.x,-n.y,0)),distance=10)
