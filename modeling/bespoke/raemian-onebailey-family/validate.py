...
assert len(owned)==5
assert B['coverage']['modeledResidentialCount']==5 and B['coverage']['officialResidentialCount']==23
expected={101,102,121,122,123}
assert {int(a['id'].split('-')[-1]) for a in B['assets']}==expected
for a in B['assets']:assert a['supersedes']==['fallback-onebailey-'+a['id'].split('-')[-1]]
q={'siteId':B['siteId'],'assetCount':len(res),'residentialCount':5,'wholeComplexComplete':False,'coverage':B['coverage'],'uniqueSourceOwnership':True,'assets':res,'totalBytes':sum(r['bytes'] for r in res),'totalTriangles':sum(r['triangles'] for r in res)}
