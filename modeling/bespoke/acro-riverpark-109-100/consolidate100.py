...
assert src.name.startswith('Acro100')
groups=collections.defaultdict(list)
for o in src.objects:
 if o.type=='MESH':groups[o.data.materials[0]].append(o)
es=bpy.data.scenes.new('Acro100_export_only');bpy.context.window.scene=es
counts={}
for mat,os in groups.items():
 ...
 me=bpy.data.meshes.new('100 '+mat.name);me.from_pydata(vs,[],fs);me.materials.append(mat);me.update();ob=bpy.data.objects.new(me.name,me);es.collection.objects.link(ob);ob.select_set(True);ob['source_semantic_objects']=len(os);counts[mat.name]=len(os)
