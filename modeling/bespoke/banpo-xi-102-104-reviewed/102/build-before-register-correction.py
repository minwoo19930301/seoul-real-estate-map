...
for prior in list(bpy.data.scenes):
 if prior.name=='Banpo Xi 102 pilot':
  bpy.data.batch_remove(ids=list(prior.objects))
  bpy.data.scenes.remove(prior)
s=bpy.data.scenes.new('Banpo Xi 102 pilot');bpy.context.window.scene=s
s['reference_scope']='KB uncredited built complex photo; tower identity and orientation unverified; 102 facade and upper step are inference'
s['height_basis']='81.0m and 29 floors OSM local mapping; no individual register confirmation; lower wing inferred'
objs=[]
def mat(n,c,metal=0):
 m=bpy.data.materials.new('BX102 '+n);m.diffuse_color=(*c,1);m.use_nodes=True;q=m.node_tree.nodes.get('Principled BSDF');q.inputs['Base Color'].default_value=(*c,1);q.inputs['Metallic'].default_value=metal;q.inputs['Roughness'].default_value=.3 if metal else .72;return m
...
def mesh(n,v,f,m):
 me=bpy.data.meshes.new(n);me.from_pydata(v,[],f);me.update();o=bpy.data.objects.new('BX102 '+n,me);s.collection.objects.link(o);me.materials.append(m);objs.append(o);return o
...
bpy.ops.export_scene.gltf(filepath=str(P/'banpo-xi-102.glb'),use_selection=True,use_active_scene=True,export_format='GLB',export_yup=True)
...
s.world=bpy.data.worlds.new('BX102 review world');s.world.use_nodes=True;s.world.node_tree.nodes['Background'].inputs[0].default_value=(.72,.77,.82,1);s.world.node_tree.nodes['Background'].inputs[1].default_value=.75
ld=bpy.data.lights.new('BX102 sun','SUN');lo=bpy.data.objects.new('BX102 sun',ld);s.collection.objects.link(lo);lo.rotation_euler=(.45,-.4,-.6);ld.energy=2.3;s.view_settings.view_transform='AgX'
for name,uvloc,z in [('front',(75,-155),80),('opposite',(-75,155),80),('side',(170,50),80),('roof',(65,-85),210)]:
 ca=bpy.data.cameras.new('BX102 '+name);ob=bpy.data.objects.new('BX102 '+name,ca);s.collection.objects.link(ob);x,y=xy(*uvloc);ob.location=(x,y,z);ob.rotation_euler=(Vector((0,0,40.5))-ob.location).to_track_quat('-Z','Y').to_euler();ca.type='ORTHO';ca.ortho_scale=98.82;s.camera=ob;s.render.filepath=str(P/(name+'.png'));bpy.ops.render.render(write_still=True)
...
bpy.data.libraries.write(str(P/'banpo-xi-102-authored.blend'),{s},fake_user=True,compress=True)
