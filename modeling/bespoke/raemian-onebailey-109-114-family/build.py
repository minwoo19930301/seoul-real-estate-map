...
old=bpy.data.scenes.get('OneBailey109to114_authoring')
if old:
...
scene=bpy.data.scenes.new('OneBailey109to114_authoring');bpy.context.window.scene=scene
scene.world=bpy.data.worlds.new('OneBailey109to114_world')
def coll(name):
...
def facade(f,p,h,floors):
 ...
 for ix,(a,b) in enumerate(zip(p,p[1:]+p[:1])):
  ...
  for j in range(N):
   ...
   # 114's road-facing service plane is visibly blank in official DJI_0062.
   # Face orientation and extent are approximate; hidden faces remain inferred.
   if T['number']==114 and L>23 and abs(a[1]-b[1])<2 and (a[1]+b[1])/2 < -10 and j in [1,2]:mi=7;holes=[]
...
scene.camera=bpy.data.objects['109_NE'];bpy.data.libraries.write(str(OUT/'raemian-onebailey-109-114.blend'),{scene},fake_user=True)
