...
R=json.loads((OUT/'recipe.json').read_text());T=next(t for t in R['towers'] if t['number']==114)
scene=bpy.data.scenes.get('OneBailey109to114_authoring');assert scene
bpy.context.window.scene=scene
C=next(c for c in scene.collection.children if c.name=='114_retained_individual_massing')
...
M.append(mat('114 medium gray photo wall',(.37,.385,.38),.8))
M.append(mat('114 dark brown photo service block',(.16,.125,.14),.8))
m=Mesh('114_editable_massing');f=Mesh('114_pale_bays_navy_shafts');r=Mesh('114_glass_roof_crown')
p=ensureccw(T['localRing']);H=T['registerHeightM'];top=H-6.4
m.prism(p,0,3.5,4);m.prism(p,3.5,top,4);facade(f,p,top,27)
a,b=p[1],p[2]
...
f.edge(lerp(a,b,.47),lerp(a,b,.79),0,31.0,12,1.0,.75)
...
q=rect([-9,-9,7,5]);m.prism(q,top,H-.35,12);r.prism(q,H-.35,H,5)
...
def glyph(text,t,z,size,out):
 cv=bpy.data.curves.new('114_photo_letter','FONT');cv.body=text;cv.size=size;cv.align_x='CENTER';cv.extrude=.015
 ob=bpy.data.objects.new('114_photo_letter',cv);C.objects.link(ob)
...
for i,ch in enumerate('ONEBAILEY'):glyph(ch,.48,top-10-i*2.6,2.1,.93)
glyph('114',.64,23.3,1.5,1.80)
...
for ob in obs:ob.location.x=575
# Store only this family scene; other scenes and completed115–120 are neither altered nor saved.
bpy.data.libraries.write(str(OUT/'raemian-onebailey-109-114.blend'),{scene},fake_user=True)
...
print('114-only photo correction exported; existing109–113 geometry retained')
