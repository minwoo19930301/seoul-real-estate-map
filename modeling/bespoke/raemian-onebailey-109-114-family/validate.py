...
with bpy.data.libraries.load(str(O/'raemian-onebailey-109-114.blend'),link=False) as (src,dst):
 standalone={'scenes':list(src.scenes),'objectNames':list(src.objects),'meshCount':len(src.meshes),'sceneCount':len(src.scenes)}
assert standalone['sceneCount']==1 and standalone['scenes']==['OneBailey109to114_authoring']
