...
P=pathlib.Path(__file__).parent;D=json.loads((P/'source-data.json').read_text());old=bpy.context.scene;scene=bpy.data.scenes.new('Raemian_Prestige_126_photo_authored_v2');bpy.context.window.scene=scene
H=D['official_height_m'];deck=64.4;facade_top=64.1;step=facade_top/23;base=step*3
scene['anchor_lonlat']=D['anchor_lonlat'];scene['footprint_id']=D['footprint_id'];scene['register_id']=D['register_id'];scene['official_height_m']=H;scene['osm_height_m']=65;scene['reference_scope']=D['reference_scope'];scene['height_policy']=D['height_policy'];scene['supersedes']='fallback-prestige-126'
...
try:
 ...
 for ring_index,r in enumerate(D['rings_m']):
  ...
  for index,(a,b) in enumerate(zip(r,r[1:]+r[:1])):
   ...
   panel('upper ivory crown frieze',.5,64.23,length,.28,.10,trim,.14)
   panel('recessed parapet warm wall',.5,64.77,length,.70,-.12,ivory,.20)
   panel('gold cream top coping',.5,65.22,length+.04,.16,-.10,gold,.31)
   if broad:
    for t in [.025,.975]:
     panel('small crown cream block',t,64.95,.32,.66,.035,trim,.20)
     panel('small gold cream crown accent',t,65.24,.35,.12,.035,gold,.21)
 # Source-contained compact plant block, subordinate to crown; dimensions are inferred.
 center=D['roof_center'];size=D['roof_size'];prism('low roof plant housing',(*center,64.65),(*size,.50),taupe)
 prism('plant cream coping',(*center,64.94),(*size,.08),trim)
 objects=[]
 ...
 report={'assetId':D['assetId'],'mesh_objects':len(objects),'min_z':min(v.co.z for o in objects for v in o.data.vertices),'max_z':max(v.co.z for o in objects for v in o.data.vertices),'register_height':65.3,'osm_height':65,'floor_count':23,'prior_scene':old.name,'created_scene':scene.name,'reference_scope':D['reference_scope'],'supersedes':['fallback-prestige-126']};assert abs(report['max_z']-65.3)<1e-4 and report['min_z']==0
