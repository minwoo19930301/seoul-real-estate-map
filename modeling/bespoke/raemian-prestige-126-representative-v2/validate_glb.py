...
v=np.concatenate(allv);t=np.concatenate(allt);source=shape(d['geometry_m']);bottom=t[np.max(np.abs(t[:,:,2]),axis=1)<1e-5];top=t[np.max(np.abs(t[:,:,2]-64.4),axis=1)<1e-4];base=unary_union([Polygon(q[:,:2]) for q in bottom]);roof=unary_union([Polygon(q[:,:2]) for q in top]);base_error=base.symmetric_difference(source).area;roof_error=roof.symmetric_difference(source).area
r={'assetId':d['assetId'],'finite':bool(np.isfinite(v).all()),'local_ground_m':float(v[:,2].min()),'max_height_m':float(v[:,2].max()),'official_recorded_height_m':65.3,'osm_height_m':65,'base_union_error_m2':base_error,'constrained_roof_union_error_m2':roof_error,'roofdeck_m':64.4,'no_fan_bridges':roof_error<.001,'source_ring_vertices_at_grade':all(any(abs(q[0]-x)<1e-4 and abs(q[1]-y)<1e-4 and abs(q[2])<1e-5 for q in v) for ring in d['rings_m'] for x,y in ring),'images':len(g.get('images',[])),'scenes':len(g['scenes']),'triangles':len(t),'vertices':len(v)}
print(r)
assert r['finite'] and r['local_ground_m']==0 and abs(r['max_height_m']-65.3)<1e-4 and base_error<.001 and roof_error<.001 and r['source_ring_vertices_at_grade'] and r['images']==0 and r['scenes']==1
