...
P=pathlib.Path(__file__).parent;D=json.loads((P/'source-data.json').read_text());regions=json.loads((P/'regions.json').read_text());old=bpy.context.scene;before={s.name:sorted(o.name for o in s.objects) for s in bpy.data.scenes};s=bpy.data.scenes.new('Raemian LaClassy102 representative');bpy.context.window.scene=s
...
def mat(n,c,metal=0,rough=.65):
 m=bpy.data.materials.new('LaClassy102 '+n);m.diffuse_color=(*c,1);m.use_nodes=True;q=m.node_tree.nodes.get('Principled BSDF');q.inputs['Base Color'].default_value=(*c,1);q.inputs['Metallic'].default_value=metal;q.inputs['Roughness'].default_value=rough;return m
...
# LaClassy: stone panels rather than continuous curtain wall; pronounced white window-bank frames.
charcoal=mat('deep charcoal vertical window surrounds',(.17,.20,.21));roles=[]
...
for ei,(a,b) in enumerate(zip(r,r[1:]+r[:1])):
 ...
 main=L>16;side=L<7
 roles.append({'edge':ei,'length_m':L,'role':'large white framed window banks with dark vertical surrounds' if main else 'pale stone/service window facade','basis':'own edge sizes; complex-photo-inference, not individually observed102orientation'})
 face(.5,body/2,L,body,.012,white,depth=.024)
 for f in range(1,FLOORS*2):face(.5,f*body/(FLOORS*2),L-.08,.018,.031,metal,depth=.01)
 for j in range(1,max(2,round(L/1.6))):face(j/max(2,round(L/1.6)),body/2,.014,body,.031,metal,depth=.01)
 face(.5,1.5*pitch,L,3*pitch,.04,base,depth=.08)
 if main:
  for start,end in [(.08,.44),(.56,.92)]:
   bw=(end-start)*L;mid=(start+end)/2
   face(mid,body/2,bw,body,.055,charcoal,depth=.07)
   for f in range(FLOORS):
    z=(f+.56)*pitch;ww=bw-.55
    face(mid,z,ww,.49*pitch,.11,glass,depth=.045)
    for q in [-1/3,0,1/3]:face(mid+q*ww/L,z,.06,.49*pitch,.16,charcoal,depth=.055)
    face(mid,f*pitch+.38,bw,.76,.17,metal,depth=.12)
    face(mid,f*pitch+.83,bw,.18,.23,white,depth=.16)
    face(mid,z,.42,.49*pitch,.23,white,depth=.18)
    face(mid+.18*bw/L,z,.24,.49*pitch,.18,charcoal,depth=.10)
   for t in [start,end]:face(t,body/2,.72,body,.23,white,depth=.18)
   face(mid,body-.25,bw+.55,.50,.19,white,depth=.16)
  face(.5,body/2,L*.085,body,.065,charcoal,depth=.075)
  for f in range(FLOORS):face(.5,(f+.57)*pitch,L*.023,.38*pitch,.12,glass,depth=.055)
 else:
  count=1 if side else 2
  for j in range(count):
   t=(j+1)/(count+1)
   for f in range(FLOORS):
    z=(f+.57)*pitch;ww=min(.72,L*.11);hh=pitch*.40
    face(t,z,ww+.08,hh+.08,.045,charcoal,depth=.06);face(t,z,ww,hh,.085,glass,depth=.04)
  if not side:
   t=.78;face(t,body/2,.23,body,.065,metal,depth=.04)
 face(.5,body-.16,L,.32,.07,white,depth=.10)
...
for m,(v,f) in batches.items():
 me=bpy.data.meshes.new('LaClassy102 editable '+m.name);me.from_pydata(v,[],f);me.update();ob=bpy.data.objects.new('LaClassy102 editable '+m.name,me);s.collection.objects.link(ob);me.materials.append(m);objs.append(ob)
...
bpy.context.view_layer.objects.active=objs[0];bpy.ops.export_scene.gltf(filepath=str(P/'raemian-la-classy-102.glb'),use_selection=True,use_active_scene=True,export_format='GLB',export_yup=True)
s.render.engine='CYCLES';s.cycles.samples=20;s.render.resolution_x=850;s.render.resolution_y=1100;s.render.resolution_percentage=100;s.world=bpy.data.worlds.new('LaClassy102 world');s.world.use_nodes=True;s.world.node_tree.nodes['Background'].inputs[0].default_value=(.72,.77,.82,1);s.world.node_tree.nodes['Background'].inputs[1].default_value=.75
ld=bpy.data.lights.new('LaClassy102 sun','SUN');lo=bpy.data.objects.new('LaClassy102 sun',ld);s.collection.objects.link(lo);lo.rotation_euler=(.45,-.4,-.6);ld.energy=2.3;s.view_settings.view_transform='AgX'
for name,(u,v),z in [('front',(75,-155),H),('opposite',(-75,155),H),('side',(170,50),H),('roof',(65,-85),H*2.55)]:
 ca=bpy.data.cameras.new('LaClassy102 '+name);ob=bpy.data.objects.new('LaClassy102 '+name,ca);s.collection.objects.link(ob);ob.location=(u*co-v*si,u*si+v*co,z);ob.rotation_euler=(Vector((0,0,H/2))-ob.location).to_track_quat('-Z','Y').to_euler();ca.type='ORTHO';ca.ortho_scale=H*1.23;inv=ob.rotation_euler.to_matrix().transposed();q=[inv@(o.matrix_world@vv.co-ob.location) for o in objs for vv in o.data.vertices];ca.ortho_scale=max(ca.ortho_scale,2*max(abs(v.y) for v in q)*1.12,2*max(abs(v.x) for v in q)/(850/1100)*1.12);s.camera=ob;s.render.filepath=str(P/(name+'.png'));bpy.ops.render.render(write_still=True)
vs=[o.matrix_world@v.co for o in objs for v in o.data.vertices];report={'source_id':D['id'],'floors':D['floors'],'register_height_m':D['register_height_m'],'osm_height_m':D['osm_height_m'],'height_status':D['height_status'],'uniform_height_assumption':True,'bounds_xyz':[[min(v[i] for v in vs) for i in range(3)],[max(v[i] for v in vs) for i in range(3)]],'mesh_count':len(objs),'image_nodes':0,'finite':all(math.isfinite(c) for v in vs for c in v)};assert abs(report['bounds_xyz'][0][2])<1e-5 and abs(report['bounds_xyz'][1][2]-H)<1e-4 and report['finite'];(P/'validation.json').write_text(json.dumps(report,indent=2));bpy.data.libraries.write(str(P/'raemian-la-classy-102-authored.blend'),{s},fake_user=True,compress=True);bpy.context.window.scene=old;bpy.data.batch_remove(ids=list(s.objects));bpy.data.scenes.remove(s);after={s.name:sorted(o.name for o in s.objects) for s in bpy.data.scenes};assert before==after;(P/'scene-preservation.json').write_text(json.dumps({'equal':before==after,'initial_scene':old.name,'before':before,'after':after},indent=2));print(json.dumps(report))
