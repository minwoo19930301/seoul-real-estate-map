...
P=pathlib.Path(__file__).parent;D=json.loads((P/'source-data.json').read_text());old=bpy.context.scene;scene=bpy.data.scenes.new('Raemian_Prestige_109_reviewed_family');bpy.context.window.scene=scene
H=D['official_height_m'];deck=(H-0.9);facade_top=(H-1.2);step=facade_top/D['floors'];base=step*3
scene['anchor_lonlat']=D['anchor_lonlat'];scene['footprint_id']=D['footprint_id'];scene['register_id']=D['register_id'];scene['official_height_m']=H;scene['osm_height_m']=D['osm_height_m'];scene['reference_scope']=D['reference_scope'];scene['height_policy']=D['height_policy'];scene['supersedes']='fallback-prestige-109';scene['inferredFromAssetIds']='bespoke-raemian-prestige-126';scene['ancestor_glb_sha256']=D['ancestor_glb_sha256'];scene['uniform_height_assumption']=True
