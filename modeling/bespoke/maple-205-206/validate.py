...
for sample in B['windowSamples']:
 ...
 assert hit[0] and '_glass' in hit[4].active_material.name,(sample,str(hit));rays.append({**sample,'firstHit':hit[4].name,'material':hit[4].active_material.name})
...
(O/f'geometry-validation-{NUMBER}.json').write_text(json.dumps({'status':'PASS','boundsBlenderENU':bounds,'components':checks,'windowRays':rays,'roofFootprintSupportChecks':len(B['roofSupport']),'noImagesInBlend':True,'scope':'Geometry and window visibility checks; representative family inference, not individual all-face photo validation.'},indent=2));bpy.ops.wm.save_as_mainfile(filepath=str(O/f'maple-{NUMBER}.blend'));print('VALIDATION_PASS',NUMBER,len(checks),len(rays),bounds)
