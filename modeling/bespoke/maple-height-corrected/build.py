"""Individual208/209/212 corrected volumes and evidence-authored edge programs via liveMCP."""
import bpy,bmesh,json,math
from pathlib import Path
from mathutils import Vector
from mathutils.geometry import tessellate_polygon
OUT=Path('/Users/hyemini/Documents/Codex/2026-09-08/seoul-elevation-local/data/model-source/bespoke/maple-height-corrected');D=json.loads((OUT/'authored-input.json').read_text())
...
for A in D['assets']:
 ...
 for e,(a,b,prog,h) in enumerate(zip(V,V[1:]+V[:1],A['faces'],edgeheights)):
  ...
  wing=0 if len(ring)==4 or e in [0,1,5] else 1;rows=A['wingFloors'][wing]-A['modeledBaseFloors'];step=(h-base)/rows
  for f in range(rows):
   z=base+f*step
 ...
 for o in objs:o['dong']=dong;o['basis']='Numbered Xi plan, opposing2025aerials andcompletedphotos';o['limits']=A['heightStatus']
 ...
 exports.append({'id':A['id'],'nameKo':'메이플자이 '+str(dong)+'동','file':A['id']+'.glb','blendSource':'maple-height-corrected.blend','coordinate':A['coordinate'],'category':'residential-apartment','footprintIds':[],'supersedes':A['supersedes'],'referenceUrl':D['sources'][1]['url'],'components':[o.name for o in objs],'uncertainties':D['uncertainties'],'minZoom':15})
 dx=(A['coordinate']['lon']-anchor['lon'])*111320*math.cos(math.radians(anchor['lat']));dy=(A['coordinate']['lat']-anchor['lat'])*111320
 exports[-1].update({'sourceHeightM':A['sourceHeightM'],'heightM':max(v.co.z for o in objs for v in o.data.vertices),'buildingFacts':{'floors':A['floors'],'heightM':A['sourceHeightM'],'sourceHeightM':A['sourceHeightM'],'source':'Seoul official building register snapshot2026-09-08','registerId':A['registerRow']['id'],'sourceHash':A['registerRow']['source_hash']},'rootApproval':'pending','heightDatumStatus':A['heightStatus'],'wingFloors':A['wingFloors']})
...
scene.camera=bpy.data.objects['208_east'];bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'maple-height-corrected.blend'))
B={'siteId':D['siteId'],'sources':D['sources'],'assets':exports,'places':[{'id':'bespoke-maple-next-towers','name':'메이플자이 동별 외관','subtitle':'208·209·212동 높이 검토안','center':[127.0142,37.5127],'zoom':17,'source_url':D['sources'][1]['url']}],'recipeFiles':['authored-input.json']};(OUT/'bundle.json').write_text(json.dumps(B,ensure_ascii=False,indent=2)+'\n');print('MAPLE_NEXT_AUTHORED',len(exports),'assets',sum(len(x) for x in assetobjects.values()),'namedcomponents')
