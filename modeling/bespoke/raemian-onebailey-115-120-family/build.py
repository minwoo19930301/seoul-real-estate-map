...
# Retain pre-existing scenes intact; author in a separate scene within this same MCP process.
old=bpy.data.scenes.get('OneBailey115to120_authoring')
if old:
...
for datablocks in [bpy.data.cameras,bpy.data.meshes]:
 for block in list(datablocks):
  if block.users==0 and block.name[:3] in ['115','116','117','118','119','120']:datablocks.remove(block)
scene=bpy.data.scenes.new('OneBailey115to120_authoring');bpy.context.window.scene=scene
scene.world=bpy.data.worlds.new('OneBailey115to120_world')
def coll(name):
...
def facade(f,p,h,floors):
 ...
 for ix,(a,b) in enumerate(zip(p,p[1:]+p[:1])):
  ...
  for j in range(N):
   ...
   # Partial115 and representative family photos show large blank service/pale planes.
   # Face orientation and extent are approximate; hidden faces remain inferred.
   if L>18 and abs(a[1]-b[1])<2 and (a[1]+b[1])/2 < -6 and j in [1]:mi=7;holes=[]
   if L>18 and abs(a[0]-b[0])<2 and j%3==1:mi=5;holes=[]
   if L>8 and L<13 and ix%4==1:mi=7 if T['number']==115 else 5;holes=[]
...
for T in R['towers']:
 ...
 C=coll(f'{no}_retained_individual_massing');m=Mesh(f'{no}_editable_massing');f=Mesh(f'{no}_pale_bays_navy_shafts');r=Mesh(f'{no}_glass_roof_crown')
 p=ensureccw(T['localRing']);s=T['split'];high=clip(p,s['axis'],s['cut'],s['highSide']);low=clip(p,s['axis'],s['cut'],-s['highSide']);H=T['registerHeightM'];top=H-6.4;lh=3.5+(s['lowFloors']-1)*3.05
 m.prism(p,0,3.5,4)
 for pp,hh,fl in [(high,top,T['registerFloors']),(low,lh,s['lowFloors'])]:
  if len(pp)<3:continue
  m.prism(pp,3.5,hh,4);facade(f,pp,hh,fl);r.prism(pp,hh,hh+.12,9);roofrail(r,pp,hh+.12)
 crown(m,f,r,T,top,H)
 ang=math.radians(T['angleDeg']);c,sn=math.cos(ang),math.sin(ang)
 for mesh in [m,f,r]:mesh.v=[(x*c-y*sn,x*sn+y*c,z) for x,y,z in mesh.v];mesh.obj(C)
 bpy.ops.object.select_all(action='DESELECT');obs=list(C.objects)
 for ob in obs:ob.select_set(True)
 bpy.context.view_layer.objects.active=obs[0];file=T['id']+'.glb';bpy.ops.export_scene.gltf(filepath=str(OUT/file),export_format='GLB',use_selection=True,use_active_scene=True,export_apply=True,export_yup=True,export_texcoords=False,export_cameras=False,export_lights=False)
 ...
 checks.append({'id':T['id'],'blenderBounds':[lo,hi],'triangles':tri,'heightErrorM':abs(hi[2]-H),'sha256':hashlib.sha256((OUT/file).read_bytes()).hexdigest(),'sourceId':T['sourceId'],'sourceRingRetained':True})
 assets.append({'id':T['id'],'nameKo':f'래미안원베일리 {no}동','file':file,'blendSource':'raemian-onebailey-115-120.blend','coordinate':dict(zip(['lon','lat'],T['coordinate'])),'category':'apartment','district':'서초구','footprintIds':[T['sourceId']],'supersedes':([f'fallback-onebailey-{no}'] if no>=117 else []),'referenceUrl':R['sources'][0]['url'],'components':['exact individual source floorplate','registered legal maximum height','individually inferred main tower and lower attached wing','pale vertical residential bay frames and navy wall panels','inferred glazed rooftop enclosure'],'modelingBasis':'representative-photo-inference','inferredFromAssetIds':['bespoke-raemian-onebailey-101'],'inferenceScope':T['inference'],'publicationDependency':'bespoke-raemian-onebailey-101','floors':T['registerFloors'],'households':T['households'],'heightBasis':'Exact per-building register maximum; wing and roof enclosure heights inferred within that maximum.','uncertainties':['Wing split and heights are visual estimates, not floor-specific cadastral facts.','Hidden facades and glazing divisions inferred from representative photos.','No invented bridges.']})
 # Store in an easy review arrangement while keeping GLBs anchored to each exact centroid.
 dx=(no-115)*115
 for ob in obs:ob.location.x=dx
 g=Mesh(f'{no}_review_ground');g.box((dx,0,-.55),(110,110,1),6);g.obj(REVIEW)
 for label,v in [('NE',(1,1,.7)),('SW',(-1,-1,.65)),('NW',(-1,1,.6)),('roof',(0,0,2))]:
  cam=bpy.data.cameras.new(f'{no}_{label}');ob=bpy.data.objects.new(f'{no}_{label}',cam);REVIEW.objects.link(ob);target=Vector((dx,0,H*.47));ob.location=target+Vector(v)*180;ob.rotation_euler=(target-ob.location).to_track_quat('-Z','Y').to_euler();cam.type='ORTHO';cam.ortho_scale=max(H*1.2,105)
scene=bpy.context.scene;scene.render.engine='CYCLES';scene.cycles.samples=12;scene.cycles.use_denoising=True;scene.render.resolution_x=850;scene.render.resolution_y=950;scene.render.resolution_percentage=100;scene.view_settings.view_transform='AgX';scene.world.use_nodes=True;scene.world.node_tree.nodes.get('Background').inputs[0].default_value=(.70,.78,.85,1);scene.world.node_tree.nodes.get('Background').inputs[1].default_value=.8
sun=bpy.data.lights.new('Daylight','SUN');ob=bpy.data.objects.new('Daylight',sun);REVIEW.objects.link(ob);ob.rotation_euler=(.35,-.6,-.5);sun.energy=2;sun.angle=.15
scene.camera=next(o for o in scene.objects if o.type=='CAMERA' and o.name.startswith('115_NE'));bpy.data.libraries.write(str(OUT/'raemian-onebailey-115-120.blend'),{scene},fake_user=True)
(OUT/'geometry-check.json').write_text(json.dumps(checks,indent=2));(OUT/'bundle.json').write_text(json.dumps({'siteId':R['siteId'],'sources':R['sources'],'assets':assets,'recipeFiles':['build.py','recipe.json','source-register.json','numbered-source-mapping.json','render.py','validate.py','REFERENCE-NOTES.md','family-lineage.json','geometry-validation.json']},ensure_ascii=False,indent=2));print(json.dumps(checks))
