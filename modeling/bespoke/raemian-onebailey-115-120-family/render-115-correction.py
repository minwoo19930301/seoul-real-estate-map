...
O=Path(__file__).resolve().parent;s=bpy.data.scenes['OneBailey115to120_authoring'];bpy.context.window.scene=s;records=[]
for cam in sorted([o for o in s.objects if o.type=='CAMERA' and o.name.startswith('115_')],key=lambda o:o.name):
...
(O/'render-115-correction-records.json').write_text(json.dumps(records,indent=2));print(records)
