...
scene=bpy.data.scenes.new('Acro102_representative_revised');bpy.context.window.scene=scene
scene['prior_scene_preserved']=old.name
scene['reference_scope']='Complex-level official photo inference; individual 102 photograph identity unverified.'
scene['height_basis']='OpenStreetMap estimate, not verified legal building-register height';scene['inferredFromAssetIds']='bespoke-acro-riverpark-109';scene['rings_m']=json.dumps(D['rings_m']);scene['anchor_lonlat']=D['anchor_lonlat'];scene['footprint_id']=D['id']
def mat(n,c,metal=0):
...
for r in D['rings_m']:
 N=len(r);H=D['height_m'];floor=(H-1.1)/D['num_floors']
 # Exact source footprint, no rectangular replacement.
 vs=[(x,y,z) for z in [0,H-1.1] for x,y in r]
 o=mesh('102 exact OSM footprint pale mass',vs,[tuple(reversed(range(N))),tuple(range(N,N*2))]+[(i,(i+1)%N,(i+1)%N+N,i+N) for i in range(N)],white)
 o['footprint_id']=D['id'];o['anchor_lonlat']=D['anchor_lonlat'];o['floors']=D['num_floors'];o['height_source']='OpenStreetMap';o['source_ring_m']=json.dumps(r)
 ...
 for i,(a,b) in enumerate(zip(r,r[1:]+r[:1])):
  ...
  face('102 charcoal 3-storey plinth',.5,floor*1.5,L,floor*3,.10,dark)
  if L<2.3:continue
  ...
  for j in range(count):
   ...
   if broad and not pale_wing and j<count-1:face('102 full-height glazed vertical field',t,(H+floor*3)/2,w,H-floor*3-2,.12,glass)
   for f in range(D['num_floors']):
    z=(f+.5)*floor
    face('102 window broad' if broad else '102 recessed window slot',t,z,w,floor*(.70 if pale_wing else .78),.23,glass)
    if broad:
     face('102 horizontal pale slab band',t,z-floor*.5,w+.12,.30,.48,white)
     if not pale_wing:face('102 dark recessed spandrel',t,z-floor*.39,w,.36,.30,dark)
     face('102 horizontal silver transom',t,z-floor*.30,w,.075,.34,frame)
     face('102 window offset mullion',t-.13/count,z,.065,floor*.78,.35,frame)
     if j%3==0:
      face('102 projecting balcony sill',t,z-floor*.42,w+.18,.17,.74,white)
      face('102 balcony side return',t-w/(2*L),z,.19,floor*.80,.65,white)
   if broad:face('102 projecting pale pier',j/count+.035,(H+floor*3)/2,.32,H-floor*3,.65,white)
  face('102 roof parapet',.5,H-.55,L,1.1,.14,white)
# Low restrained roof cap inferred from complex photo; no speculative bridge/canopy.
box('102 compact roof plant cap',(*D['roof_center_inferred'],H-1.10),(7,5,1.9),roof,D['roof_axis_inferred'])
box('102 pale roof cap coping',(*D['roof_center_inferred'],H-.11),(7.6,5.6,.22),white,D['roof_axis_inferred'])
# Consolidate editable geometry by architectural material.
groups={}
...
bpy.ops.export_scene.gltf(filepath=str(P/'acro-riverpark-102.glb'),use_selection=True,use_active_scene=True,export_format='GLB',export_yup=True)
...
for name,loc in [('front',(125,-180,95)),('opposite',(-125,180,95)),('side',(180,110,90)),('roof',(110,-120,240))]:
 ca=bpy.data.cameras.new('102 '+name);co=bpy.data.objects.new('102 '+name,ca);scene.collection.objects.link(co);co.location=loc;co.rotation_euler=(Vector((0,0,H*.49))-co.location).to_track_quat('-Z','Y').to_euler();ca.type='ORTHO';ca.ortho_scale=max(H*1.2,105);scene.camera=co
...
bpy.data.libraries.write(str(P/'acro-riverpark-102-authored.blend'),{scene},fake_user=True,compress=True)
report={'tower':'102','floors':D['num_floors'],'height_m':H,'height_basis':'OpenStreetMap; not legal register','footprint_id':D['id'],'ring_vertex_counts':[len(r) for r in D['rings_m']],'anchor_lonlat':D['anchor_lonlat'],'mesh_objects':len(objs),'reference_scope':scene['reference_scope'],'original_scene_preserved':old.name,'views':['front','opposite','side','roof']}
