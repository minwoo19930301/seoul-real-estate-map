...
def main():
    ...
    if args.additional_pool and not args.allow_shortfall:
        assert len(district_counts)==25 and set(district_counts.values())=={args.per_district},district_counts
    (args.output or ROOT/('docs/landmark-candidates-expansion-matched.json' if args.additional_pool else 'docs/landmark-candidates-matched.json')).write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n')
    (args.rejections_output or ROOT/('docs/landmark-expansion-unmatched.json' if args.additional_pool else 'docs/landmark-unmatched.json')).write_text(json.dumps(missing,ensure_ascii=False,indent=2)+'\n')
