...
| `gratte-ciel` | Restore `authored-input.json`, `source-footprints.json`, `official-register-source.json`; run `gratte_ciel.py`, then `gratte_ciel_render.py` through separate MCP calls. | Four named residential towers with individual anchors. Legal heights and approximate crown envelope interpretation are distinct from a surveyed architectural top. |
| `trimage` | Restore frozen inputs; run `trimage_blender.py`, then `trimage_render.py` and regular-Python `trimage_finalize.py`. | Four separately anchored towers. Exact-parcel public register supersedes uncertain OSM heights; absent gate georeference is not invented. Preparation downloads are optional and can overwrite later face corrections. |
| `raemian-caelitus` | Restore `authored-input.json` and register rows; run `raemian_caelitus_blender.py`, then matching render/validation scripts through MCP. | Three towers with bridge/podium geometry owned by103. Preserve generic correction overlay:101 replaces `fallback-caelitus-101`, never the six-neighbor `apt-a14003002`. |
