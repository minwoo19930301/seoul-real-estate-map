...
for(const [site,count,representative] of [['songpa-helio-city',33,416],['jamsil-parkrio',59,207],['jamsil-trizium',44,344],['daechi-eunma',27,21],['godeok-gracium',21,101],['raemian-la-classy',4,102],['godeok-arteon',24,323],['godeok-central-ipark',17,512],['godeok-raemian-hillstate',27,126]]){
  test(`${site} copies retain individual ownership and one shared file`,()=>{
    ...
    for(const a of copies){
      ...
      assert.equal(a.modelInstance.sourceAssetId,source.id);assert.equal(a.modelInstance.hiddenNodes.length, ['godeok-gracium','raemian-la-classy','godeok-arteon','godeok-central-ipark','godeok-raemian-hillstate'].includes(site)?0:1);
