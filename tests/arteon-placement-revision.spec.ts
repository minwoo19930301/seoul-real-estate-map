...
test('Arteon326 revised placement draw once with old copies hidden',async({page})=>{
 test.setTimeout(180_000);
 const rows=assets.filter((a:any)=>a.sourceRecord.siteId==='godeok-arteon-placement-v2');expect(rows).toHaveLength(1);
 ...
 await page.screenshot({path:'tests/screenshots/shared-apartment-instances/godeok-arteon-placement-v2.png'});
 writeFileSync('tests/screenshots/shared-apartment-instances/godeok-arteon-placement-v2.json',JSON.stringify({rows:rows.map((a:any)=>({id:a.id,revision:a.sourceRecord.placementRevision})),active:state.models.filter((m:any)=>m.active).map((m:any)=>m.id),errors},null,2)+'\n');
});
