...
test('six individually rebuilt sites replace only their owned geometry and retain neighbouring buildings', async ({page}) => {
  ...
  for(const sample of [
    ...
    {site:'maple',center:[127.01307870765513,37.51157245048949],zoom:16.4,ids:['bespoke-maple-xi-210','bespoke-maple-xi-211'],source:[],old:['maple-xi-210','maple-xi-211']},
    ...
    if(sample.site==='maple') {
      ...
      expect(towers.filter((m:any)=>m.id.startsWith('bespoke-'))).toHaveLength(2);
      expect(towers.filter((m:any)=>m.id.startsWith('maple-'))).toHaveLength(27);
    }
