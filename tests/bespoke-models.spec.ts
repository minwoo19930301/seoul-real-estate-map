...
test('eight individually rebuilt sites replace only their owned geometry and retain neighbouring buildings', async ({page}) => {
  ...
  for(const sample of [
    ...
    {site:'maple',center:[127.01307870765513,37.51157245048949],zoom:16.4,ids:[...Array.from({length:14},(_,i)=>101+i),...Array.from({length:15},(_,i)=>201+i)].map(n=>'bespoke-maple-xi-'+n+([208,209,212,213].includes(n)?'-corrected':'')),source:[],old:[...Array.from({length:14},(_,i)=>101+i),...Array.from({length:15},(_,i)=>201+i)].map(n=>'maple-xi-'+n).concat([208,209,212,213].map(n=>'bespoke-maple-xi-'+n))},
    ...
    if(sample.site==='maple') {
      ...
      expect(towers.filter((m:any)=>m.id.startsWith('bespoke-'))).toHaveLength(29);
      expect(towers.filter((m:any)=>m.id.startsWith('maple-'))).toHaveLength(0);
    }
