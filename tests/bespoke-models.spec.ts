...
test('eight individually rebuilt sites replace only their owned geometry and retain neighbouring buildings', async ({page}) => {
  ...
  for(const sample of [
    ...
    {site:'maple',center:[127.01307870765513,37.51157245048949],zoom:16.4,ids:[204,207,208,209,210,211,212].map(n=>'bespoke-maple-xi-'+n).concat('bespoke-maple-xi-213-corrected'),source:[],old:[204,207,208,209,210,211,212,213].map(n=>'maple-xi-'+n).concat('bespoke-maple-xi-213')},
    ...
    if(sample.site==='maple') {
      ...
      expect(towers.filter((m:any)=>m.id.startsWith('bespoke-'))).toHaveLength(8);
      expect(towers.filter((m:any)=>m.id.startsWith('maple-'))).toHaveLength(21);
    }
