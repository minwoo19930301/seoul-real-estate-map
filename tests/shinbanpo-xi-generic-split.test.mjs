...
const source=base.assets.find(a=>a.id==='apt-a10026004');
const correction=corrections.corrections.find(c=>c.sourceId===source.id);
const proof=json('docs/model-audit/shinbanpo-xi-generic-split.json');
const bindings=json('docs/model-audit/shinbanpo-xi-fallback-bindings.json').bindings;
...
test('Shinbanpo seven partitions conserve all 3861 original triangle emissions exactly once',()=>{
 assert.equal(source.sha256,'8b9bd8a22b4347b3653b6729d81c08befafbf5eb0b46a848a95701fc251e045a');
 ...
 assert.equal(correction.assets.reduce((n,a)=>n+a.triangles,0),3861);
 ...
 assert.equal(proof.trianglesAssignedWithinUniqueSourceConvexHull,101);
 assert.ok(proof.maximumVertexDistanceOutsideSourceFootprintM>5.14&&proof.maximumVertexDistanceOutsideSourceFootprintM<5.141,'Inherited roofplant approximation is documented and retained');
});
test('Shinbanpo bindings retain seven independent source owners without a hidden compound',()=>{
 const identity=json('docs/model-audit/shinbanpo-xi-source-identity.json');
 ...
 assert.equal(identity.towers.reduce((sum,t)=>sum+Number(t.register.households),0),607);
 ...
 for(const b of bindings){
  ...
  assert.equal(b.fallbackAssetId,`fallback-shinbanpo-xi-${b.number}`);
...
test('Shinbanpo split preserves all twelve pre-existing correction records and their files',()=>{
 ...
 assert.equal(hash(JSON.stringify(sort(prior))),'48f81422848315d6d0f21730e82311a50f992fa77b90c16d38b12ed15b9e5fec');
