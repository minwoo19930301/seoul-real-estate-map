...
const sources = read('docs/model-audit/banpo-riche-source-identity.json').towers;
const byId = new Map<string, any>([...base, ...corrections.flatMap((c: any) => c.assets), ...authored].map((a: any) => [a.id, a]));
const numbers = Array.from({length: 9}, (_, i) => 101 + i);
const scenarios = [
  ...
  {name: 'all9-fallbacks', fail: numbers, raw: [] as number[]},
];

for (const scenario of scenarios) {
  test(`Banpo Riche distinct towers: ${scenario.name}`, async ({ page }) => {
    test.setTimeout(180_000);
    const family = numbers.map(number => {
      const source = sources.find((s: any) => s.number === number);
      const asset = byId.get(`bespoke-banpo-riche-${number}`);
    ...
    for (const id of failedIds) {
      await page.route(`**/models/${byId.get(id).model}`, route => route.fulfill({ status: 404, body: 'intentional Banpo Riche model failure' }));
    }
    const errors: string[] = [];
    ...
    await page.evaluate(center => (window as any).__SEOUL_MAP__.map.jumpTo({ center, zoom: 16.65, pitch: 55, bearing: 165 }), [127.0136, 37.5026]);
    ...
    expect(state.models.find((m: any) => m.id === 'apt-a13776301')?.active ?? false).toBe(false);
    expect(errors).toEqual([]);
    const directory = 'tests/screenshots/banpo-riche9'; mkdirSync(directory, { recursive: true });
